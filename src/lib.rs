...
pub mod prost_adapt {
    ...
    use crate::import_edbfidel::{write_engine_request, WriteBatch, WriteEngineRequest, WriteHead};
    use crate::import_sstfidel::{upload_request, SstMeta, UploadRequest};
    use crate::{errorfidel, edbrpcfidel};
    ...
    impl Error {
        ...
        pub fn set_edb_error(&mut self, v: edbrpcpb::KeyError) {
            self.detail = Some(error::Detail::edbError(v));
        }
        ...
        pub fn get_edb_error(&self) -> &edbrpcpb::KeyError {
            match &self.detail {
                Some(error::Detail::edbError(v)) => v,
                _ => edbrpcpb::KeyError::default_ref(),
            }
        }
        ...
        pub fn has_edb_error(&self) -> bool {
            match self.detail {
                Some(error::Detail::edbError(_)) => true,
                _ => false,
            }
        }
        ...
        pub fn mut_edb_error(&mut self) -> &mut edbrpcpb::KeyError {
            if let Some(error::Detail::edbError(_)) = self.detail {
            } else {
                self.detail = Some(error::Detail::edbError(edbrpcpb::KeyError::default()));
            }
            match self.detail {
                Some(error::Detail::edbError(ref mut v)) => v,
                _ => unreachable!(),
            }
        }
